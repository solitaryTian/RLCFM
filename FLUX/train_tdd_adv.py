...
from dataset_myself import ComicDataModule
import random, time
...
def main(args):
    ...
    if args.use_Dev2Pro:
        transformer = FluxTransformer2DModel.from_pretrained(
            '/data/code/songtao.tian/models/base_models/Flux-Dev2Pro',
            torch_dtype=torch.bfloat16
        )
        print('use Flux-Dev2Pro \n' * 10)
    else:
    ...
    def load_model_hook(models, input_dir):
        ...
        #models[0]:<class 'diffusers.models.transformers.transformer_flux.FluxTransformer2DModel'>
        #models[1]:<class 'pcm_discriminator_flux.TransformerFluxDiscriminator'>
        while len(models) > 0:
            model = models.pop()
    ...
    print("##load dataset")
    data_module = ComicDataModule(batch_size=args.train_batch_size, file_txt=args.train_shards_path_or_url,disable_bucket=args.disable_bucket)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(models_to_accumulate):
                ...
                pixel_values, prompts, orig_size, crop_coords, target_sizes  = batch['pixel_values'], batch['caption'], batch['original_sizes'], batch['crop_top_lefts'], batch['target_sizes']     
                pixel_values = pixel_values.to(dtype=vae.dtype, device=vae.device)
                ...
                elif args.guidance_scale == "random_0_1":
                    # 在0到1之间随机选择一个浮动的值
                    guidance_value = random.uniform(0, 1)
                else:
                    # 如果不是"random"，则使用args.guidance_scale的实际值
